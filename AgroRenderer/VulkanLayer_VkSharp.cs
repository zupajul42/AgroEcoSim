...
public static unsafe class VkSharp
{
        ...
        MemUtils.Arena scratch, DebugMessengerCreateInfo? debugCreateInfo = null)
    {
        ...
        var layerCount = (uint)createInfo.EnabledLayerNames.Length;
        vkCreateInfo->enabledLayerCount = layerCount;
        if (layerCount > 0)
        {
            var layerNames = scratch.Alloc<IntPtr>((int)layerCount);
        ...
        var extensionCount = (uint)createInfo.EnabledExtensionNames.Length;
        vkCreateInfo->enabledExtensionCount = extensionCount;
        if (extensionCount > 0)
        {
            var extensionNames = scratch.Alloc<IntPtr>((int)extensionCount);
    ...
    public static PhysicalDevices GetPhysicalDevices(Vk.VkInstance instance, MemUtils.Arena scratch)
    {
        ...
        var devices = scratch.Alloc<Vk.VkPhysicalDevice>((int)deviceCount);
        result = Vk.vkEnumeratePhysicalDevices(instance, &deviceCount, devices);
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to enumerate physical devices.");
        ...
        for (var i = 0; i < deviceCount; i++)
        {
            ...
            var deviceName = Marshal.PtrToStringAnsi((IntPtr)(&properties[i])->properties.deviceName) ?? "Unknown";
            Console.WriteLine(
        ...
        MemUtils.Arena scratch)
    {
        ...
        {
            var queueFamilyCount = scratch.Alloc<uint>();
            ...
            for (var j = 0; j < *queueFamilyCount; j++)
            {
                ...
                var presentSupport = res == Vk.VkResult.VK_SUCCESS && presentSupportBool32 == Vk.VkBool32.VK_TRUE;
                if (presentSupport)
        ...
        string[]? enabledLayerNames = null, string[]? enabledExtensionNames = null)
    {
        ...
        if (queuesToRequest.Graphics is not null)
        {
            queueCreateInfos[index] = new Vk.VkDeviceQueueCreateInfo
            {
                ...
                pNext = IntPtr.Zero,
                flags = 0,
                queueFamilyIndex = (uint)queuesToRequest.Graphics.Value,
        ...
        if (queuesToRequest.Present is not null && queuesToRequest.Present != queuesToRequest.Graphics)
        {
            queueCreateInfos[index] = new Vk.VkDeviceQueueCreateInfo
            {
                ...
                pNext = IntPtr.Zero,
                flags = 0,
                queueFamilyIndex = (uint)queuesToRequest.Present.Value,
                queueCount = 1,
                pQueuePriorities = &queuePriorities[index]
            };
            queuePriorities[index] = 1.0f; // Highest priority
        ...
        deviceCreateInfo->queueCreateInfoCount = (uint)queueCreateInfoCount;
        deviceCreateInfo->pQueueCreateInfos = queueCreateInfos;
        deviceCreateInfo->enabledLayerCount = (uint)enabledLayerNames.Length;
        deviceCreateInfo->enabledExtensionCount = (uint)enabledExtensionNames.Length;
        deviceCreateInfo->pEnabledFeatures = null; // Enable all features by default
        // Marshal enabled Layer Names
        if (deviceCreateInfo->enabledLayerCount > 0)
        {
            var layerNames = scratch.Alloc<IntPtr>((int)deviceCreateInfo->enabledLayerCount);
            for (var i = 0; i < deviceCreateInfo->enabledLayerCount; i++)
        ...
        {
            var extensionNames = scratch.Alloc<IntPtr>((int)deviceCreateInfo->enabledExtensionCount);
            for (var i = 0; i < deviceCreateInfo->enabledExtensionCount; i++)
        ...
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception($"Failed to create logical device: {result}");
        ...
        if (queuesToRequest.Graphics is not null)
        {
            ...
            Vk.vkGetDeviceQueue(*logicalDevice, (uint)queuesToRequest.Graphics.Value, 0, graphicsQueue);
        ...
        if (queuesToRequest.Present is not null)
        {
            ...
            Vk.vkGetDeviceQueue(*logicalDevice, (uint)queuesToRequest.Present.Value,
    ...
    private static Vk.VkLayerProperties[] GetAvailableDeviceLayers(Vk.VkPhysicalDevice physicalDevice,
        MemUtils.Arena scratch)
    {
        uint availableLayerCount = 0;
        var result = Vk.vkEnumerateDeviceLayerProperties(physicalDevice, &availableLayerCount, null);
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to enumerate device layer properties.");
        ...
        for (var i = 0; i < availableLayerCount; i++) availableLayersArray[i] = availableLayers[i];
    ...
    public static Vk.VkExtensionProperties[] GetAvailableDeviceExtensions(Vk.VkPhysicalDevice physicalDevice,
        MemUtils.Arena scratch)
    {
        uint availableExtensionCount = 0;
        var result =
            Vk.vkEnumerateDeviceExtensionProperties(physicalDevice, IntPtr.Zero, &availableExtensionCount, null);
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to enumerate device extension properties.");
        var availableExtensions = scratch.Alloc<Vk.VkExtensionProperties>((int)availableExtensionCount);
        result = Vk.vkEnumerateDeviceExtensionProperties(physicalDevice, IntPtr.Zero, &availableExtensionCount,
            availableExtensions);
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to enumerate device extension properties.");
        var availableExtensionsArray = new Vk.VkExtensionProperties[availableExtensionCount];
        for (var i = 0; i < availableExtensionCount; i++) availableExtensionsArray[i] = availableExtensions[i];
        return availableExtensionsArray;
    }

    public static bool CheckIfExtensionsAvailable(string[] requestedExtensions,
        Vk.VkExtensionProperties[] availableExtensions)
    {
        var allFound = true;
        var availableExtensionsStrings = availableExtensions
            .Select(availableExtension => Marshal.PtrToStringAnsi((IntPtr)availableExtension.extensionName) ?? "")
            .ToArray();
        foreach (var requestedExtension in requestedExtensions)
        {
            var found = availableExtensionsStrings.Any(availableExtensionName =>
                requestedExtension == availableExtensionName);
            if (!found)
            {
                allFound = false;
        ...
        Vk.VkPresentModeKHR? desiredPresentMode = null)
    {
        ...
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception($"Failed to create swapchain: {result}");
        ...
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to get swapchain images.");
        ...
        Vk.VkSurfaceKHR surface, MemUtils.Arena scratch)
    {
        ...
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to get surface capabilities.");
        ...
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to get surface formats.");
        ...
        result = Vk.vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface, &presentModeCount,
            presentModes);
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to get present modes.");
        ...
        for (var i = 0; i < formatCount; i++) details.Formats[i] = formats[i];

        details.PresentModes = new Vk.VkPresentModeKHR[presentModeCount];
        for (var i = 0; i < presentModeCount; i++) details.PresentModes[i] = presentModes[i];
        ...
        MemUtils.Arena scratch)
    {
        ...
        var spirvSize = (ulong)GLSLang.glslang_program_SPIRV_get_size(program);
    ...
    public static CompiledSpirv CompileShaders(string? vertexShaderSource, string? fragmentShaderSource,
        MemUtils.Arena scratch)
    {
        var vertSpirv = vertexShaderSource is not null
            ? CompileGLSLtoSPIRV(vertexShaderSource, GLSLang.glslang_stage_t.GLSLANG_STAGE_VERTEX, scratch)
            : [];
        var fragSpirv = fragmentShaderSource is not null
            ? CompileGLSLtoSPIRV(fragmentShaderSource, GLSLang.glslang_stage_t.GLSLANG_STAGE_FRAGMENT, scratch)
            : [];
        ...
        Vk.VkBufferUsageFlagBits usage, Vk.VkMemoryPropertyFlagBits properties, MemUtils.Arena scratch)
    {
        ...
        var memoryTypeIndex =
            FindSuitableMemoryTypeIndex(physicalDevice, memRequirements->memoryTypeBits, properties, scratch);
        ...
        if (Vk.vkBindBufferMemory(device, result.Buffer, result.Memory, new Vk.VkDeviceSize(0)) !=
            Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to bind buffer memory.");
        return result;
    }
    ...
    public static BufferInfo CreateVertexBufferForArray<T>(Vk.VkDevice device, Vk.VkPhysicalDevice physicalDevice,
        T[] data,
    ...
    public static BufferInfo CreateIndexBufferForArray<T>(Vk.VkDevice device, Vk.VkPhysicalDevice physicalDevice,
        T[] data,
        MemUtils.Arena scratch) where T : unmanaged
    {
    ...
    public static uint FindSuitableMemoryTypeIndex(Vk.VkPhysicalDevice physicalDevice, uint typeFilter,
        Vk.VkMemoryPropertyFlagBits properties, MemUtils.Arena scratch)
    {
        ...
        for (uint i = 0; i < memProperties->memoryTypeCount; i++)
        {
            ...
            Console.WriteLine(
                $"   Memory Type {i}: HeapIndex: 0x{memoryType.heapIndex:X}, PropertyFlags: 0x{memoryType.propertyFlags:X}");
            if ((typeFilter & (1 << (int)i)) != 0 && (memoryType.propertyFlags & properties) == properties) return i;
        }

        throw new Exception("Failed to find suitable memory type.");
    }

    private static void LoadDataToBufferUnsafe(Vk.VkDevice device, BufferInfo bufferInfo, void* data,
        Vk.VkDeviceSize size)
    {
        void* mappedData = null;
        if (Vk.vkMapMemory(device, bufferInfo.Memory, new Vk.VkDeviceSize(0), size, 0, &mappedData) !=
            Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to map buffer memory.");
    ...
    {
        var size = new Vk.VkDeviceSize((ulong)(data.Length * sizeof(T)));
        if (size.size > bufferInfo.Size) throw new Exception("Data size exceeds buffer size.");
    ...
    public static Vk.VkSemaphore[] CreateSemaphores(Vk.VkDevice device, int count, MemUtils.Arena scratch)
    {
        ...
        for (var i = 0; i < count; i++) semaphores[i] = CreateSemaphore(device, scratch);
        return semaphores;
    }
    ...
    public static Vk.VkFence[] CreateFences(Vk.VkDevice device, int count, bool signaled, MemUtils.Arena scratch)
    {
        ...
        for (var i = 0; i < count; i++) fences[i] = CreateFence(device, signaled, scratch);
        return fences;
    }
    ...
    public static Vk.VkCommandBuffer CreateCommandBuffer(Vk.VkDevice device, Vk.VkCommandPool commandPool,
        MemUtils.Arena scratch)
    {
    ...
    public static Vk.VkCommandBuffer[] CreateCommandBuffers(Vk.VkDevice device, Vk.VkCommandPool commandPool, int count,
        MemUtils.Arena scratch)
    {
        ...
        Console.WriteLine(
            $"Allocated Command Buffers with handles from 0x{commandBuffersManaged[0].handle:X} to 0x{commandBuffersManaged[^1].handle:X}");
        ...
        Vk.VkShaderModule fragmentShader, Vk.VkFormat[] colourAttachmentFormats, MemUtils.Arena scratch)
    {
        ...
        vertexAttributeDescriptions[0].format =
            Vk.VkFormat.VK_FORMAT_R32G32_SFLOAT; // To be set based on vertex structure
        vertexAttributeDescriptions[0].offset =
            (uint)Marshal.OffsetOf<Vertex>("Position").ToInt32(); // To be set based on vertex structure
        vertexAttributeDescriptions[1].location = 1;
        vertexAttributeDescriptions[1].binding = 0;
        vertexAttributeDescriptions[1].format =
            Vk.VkFormat.VK_FORMAT_R32G32B32_SFLOAT; // To be set based on vertex structure
        vertexAttributeDescriptions[1].offset =
            (uint)Marshal.OffsetOf<Vertex>("Color").ToInt32(); // To be set based on vertex structure
        ...
        for (var i = 0; i < dynamicStatesToEnable.Length; i++) dynamicStates[i] = dynamicStatesToEnable[i];
        ...
        for (var i = 0; i < colourAttachmentFormats.Length; i++) colorFormats[i] = colourAttachmentFormats[i];
        ...
        if (Vk.vkCreateGraphicsPipelines(device, new Vk.VkPipelineCache { handle = 0 }, 1, pipelineCreateInfoMain, null,
                pipeline) != Vk.VkResult.VK_SUCCESS) throw new Exception("Failed to create graphics pipeline.");
    ...
    public static void WaitForFences(Vk.VkDevice logical_device, Vk.VkFence[] fences, MemUtils.Arena scratch,
        bool waitAll = true, ulong timeout = 1000 * 1000)
    {
        var fenceHandles = scratch.Alloc<Vk.VkFence>(fences.Length);
        for (var i = 0; i < fences.Length; i++) fenceHandles[i] = fences[i];
        var result = Vk.vkWaitForFences(logical_device, (uint)fences.Length, fenceHandles,
            waitAll ? Vk.VkBool32.VK_TRUE : Vk.VkBool32.VK_FALSE, timeout);
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception($"Failed to wait for fences: {result}");
    }

    public static void ResetFences(Vk.VkDevice logical_device, Vk.VkFence[] fences, MemUtils.Arena scratch)
    {
        var fenceHandles = scratch.Alloc<Vk.VkFence>(fences.Length);
        for (var i = 0; i < fences.Length; i++) fenceHandles[i] = fences[i];
        var result = Vk.vkResetFences(logical_device, (uint)fences.Length, fenceHandles);
        if (result != Vk.VkResult.VK_SUCCESS) throw new Exception($"Failed to reset fences: {result}");
    }

    public static uint AcquireNextImage(Vk.VkDevice device, Vk.VkSwapchainKHR swapchain,
        Vk.VkSemaphore imageAvailableSemaphore, MemUtils.Arena scratch, ulong timeout = 1000 * 1000)
    {
        var imageIndex = scratch.Alloc<uint>();
        var result = Vk.vkAcquireNextImageKHR(device, swapchain, timeout, imageAvailableSemaphore,
            new Vk.VkFence { handle = 0 }, imageIndex);
        if (result != Vk.VkResult.VK_SUCCESS && result != Vk.VkResult.VK_SUBOPTIMAL_KHR)
            throw new Exception($"Failed to acquire next image: {result}");
        return *imageIndex;
    }

    public static void ResetCommandBuffer(Vk.VkCommandBuffer commandBuffer)
    {
        var result = Vk.vkResetCommandBuffer(commandBuffer,
            Vk.VkCommandBufferResetFlagBits.VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
        if (result != Vk.VkResult.VK_SUCCESS)
            throw new Exception($"Failed to reset command buffer: {result}");
    }

    public static void BeginCommandBuffer(Vk.VkCommandBuffer commandBuffer)
    {
        var beginInfo = new Vk.VkCommandBufferBeginInfo
        {
    ...
    public static void CmdTransitionImageLayout(Vk.VkCommandBuffer commandBuffer, Vk.VkImage image,
        ImageTransitionType type)
    {
        var barrier = new Vk.VkImageMemoryBarrier
        {
            ...
            newLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_UNDEFINED,
        ...
        Vk.VkPipelineStageFlagBits sourceStage = 0;
        Vk.VkPipelineStageFlagBits destinationStage = 0;

        switch (type)
        {
            case ImageTransitionType.UndefinedToTransferDst:
                barrier.oldLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_UNDEFINED;
                barrier.newLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
                barrier.srcAccessMask = 0;
                barrier.dstAccessMask = Vk.VkAccessFlagBits.VK_ACCESS_TRANSFER_WRITE_BIT;

                sourceStage = Vk.VkPipelineStageFlagBits.VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
                destinationStage = Vk.VkPipelineStageFlagBits.VK_PIPELINE_STAGE_TRANSFER_BIT;
                break;

            case ImageTransitionType.UndefinedToColorAttachment:
                barrier.oldLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_UNDEFINED;
                barrier.newLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
                barrier.srcAccessMask = 0;
                barrier.dstAccessMask = Vk.VkAccessFlagBits.VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
            ...
            case ImageTransitionType.ColorAttachmentToPresent:
                barrier.oldLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
                barrier.newLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
                barrier.srcAccessMask = Vk.VkAccessFlagBits.VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
                barrier.dstAccessMask = 0;
                sourceStage =
                    Vk.VkPipelineStageFlagBits.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
                destinationStage = Vk.VkPipelineStageFlagBits.VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;
                break;
            case ImageTransitionType.PresentToColorAttachment:
                barrier.oldLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
                barrier.newLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
                barrier.srcAccessMask = 0;
                barrier.dstAccessMask =
                    Vk.VkAccessFlagBits.VK_ACCESS_COLOR_ATTACHMENT_READ_BIT |
                ...
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(type), type, "Unhandled image transition type");
        }
    ...
    public static void CmdBeginRendering(Vk.VkCommandBuffer commandBuffer, Vk.VkExtent2D extent,
        Vk.VkImageView colorAttachmentView, ClearValue clearColor, MemUtils.Arena scratch)
    {
        var colorAttachment = scratch.Alloc<Vk.VkRenderingAttachmentInfo>();
        colorAttachment->sType = Vk.VkStructureType.VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO;
        colorAttachment->pNext = null;
        colorAttachment->imageView = colorAttachmentView;
        colorAttachment->imageLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
        colorAttachment->resolveMode = Vk.VkResolveModeFlagBits.VK_RESOLVE_MODE_NONE;
        colorAttachment->resolveImageView = new Vk.VkImageView { handle = 0 };
        colorAttachment->resolveImageLayout = Vk.VkImageLayout.VK_IMAGE_LAYOUT_UNDEFINED;
        colorAttachment->loadOp = Vk.VkAttachmentLoadOp.VK_ATTACHMENT_LOAD_OP_CLEAR;
        colorAttachment->storeOp = Vk.VkAttachmentStoreOp.VK_ATTACHMENT_STORE_OP_STORE;
        colorAttachment->clearValue = clearColor.asVkClearValue();
        var renderingInfo = scratch.Alloc<Vk.VkRenderingInfo>();
        renderingInfo->sType = Vk.VkStructureType.VK_STRUCTURE_TYPE_RENDERING_INFO;
        renderingInfo->pNext = null;
        renderingInfo->flags = 0;
        renderingInfo->renderArea = new Vk.VkRect2D
        {
            offset = new Vk.VkOffset2D { x = 0, y = 0 },
            extent = extent
        };
    ...
    public static void CmdSetViewport(Vk.VkCommandBuffer commandBuffer, float width, float height,
        MemUtils.Arena scratch)
    {
        // Here, some magic is needed to change the Vulkan coordinate system
        // (origin in top-left corner with positive y pointing down) into the
        // default OpenGL system (origin in lower-left corner with positive y
        // pointing up). Without this you would have to change all the vertex
        // positions to avoid the scene being upside down, and have all triangles
        // flipped back-to-front in winding number.

        var viewport = scratch.Alloc<Vk.VkViewport>();
        viewport->x = 0.0f;
        viewport->y = height;
    ...
    public static void CmdSetScissor(Vk.VkCommandBuffer commandBuffer, uint width, uint height, MemUtils.Arena scratch)
    {
        var scissor = scratch.Alloc<Vk.VkRect2D>();
        scissor->offset = new Vk.VkOffset2D { x = 0, y = 0 };
        scissor->extent = new Vk.VkExtent2D { width = width, height = height };

        Vk.vkCmdSetScissor(commandBuffer, 0, 1, scissor);
    }

    public static void CmdSetCullMode(Vk.VkCommandBuffer commandBuffer, Vk.VkCullModeFlagBits cullMode)
    {
        Vk.vkCmdSetCullMode(commandBuffer, cullMode);
    }

    public static void CmdSetFrontFace(Vk.VkCommandBuffer commandBuffer, Vk.VkFrontFace frontFace)
    {
        Vk.vkCmdSetFrontFace(commandBuffer, frontFace);
    }

    public static void CmdSetPrimitiveTopology(Vk.VkCommandBuffer commandBuffer, Vk.VkPrimitiveTopology topology)
    {
        Vk.vkCmdSetPrimitiveTopology(commandBuffer, topology);
    }

    public static void CmdBindPipeline(Vk.VkCommandBuffer commandBuffer, Vk.VkPipeline pipeline)
    {
        Vk.vkCmdBindPipeline(commandBuffer, Vk.VkPipelineBindPoint.VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
    }

    public static void CmdBindVertexBuffer(Vk.VkCommandBuffer commandBuffer, Vk.VkBuffer vertexBuffer,
        MemUtils.Arena scratch)
    {
        var offsets = scratch.Alloc<Vk.VkDeviceSize>();
        offsets[0].size = 0;
        var buffers = scratch.Alloc<Vk.VkBuffer>();
        buffers[0] = vertexBuffer;
        Vk.vkCmdBindVertexBuffers(commandBuffer, 0, 1, buffers, offsets);
    }

    public static void CmdBindIndexBuffer(Vk.VkCommandBuffer commandBuffer, Vk.VkBuffer indexBuffer,
        Vk.VkIndexType indexType)
    {
        var offset = new Vk.VkDeviceSize();
        offset.size = 0;
        Vk.vkCmdBindIndexBuffer(commandBuffer, indexBuffer, offset, indexType);
    }

    public static void CmdDrawIndexed(Vk.VkCommandBuffer commandBuffer, uint indexCount)
    {
        Vk.vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);
    }

    public static void CmdEndRendering(Vk.VkCommandBuffer commandBuffer)
    {
        Vk.vkCmdEndRendering(commandBuffer);
    }

    public static void EndCommandBuffer(Vk.VkCommandBuffer commandBuffer)
    {
        var result = Vk.vkEndCommandBuffer(commandBuffer);
        if (result != Vk.VkResult.VK_SUCCESS)
            throw new Exception($"Failed to end command buffer: {result}");
    }

    public static void QueueSubmit(Vk.VkQueue queue, Vk.VkCommandBuffer[] commandBuffers,
        Vk.VkSemaphore[] waitSemaphores, Vk.VkSemaphore[] signalSemaphores, Vk.VkFence fence, MemUtils.Arena scratch)
    {
        var submitInfo = scratch.Alloc<Vk.VkSubmitInfo>();
        submitInfo->sType = Vk.VkStructureType.VK_STRUCTURE_TYPE_SUBMIT_INFO;
        submitInfo->pNext = null;
        if (waitSemaphores.Length > 0)
        {
            submitInfo->waitSemaphoreCount = (uint)waitSemaphores.Length;
            var waitSemaphoresPtr = scratch.Alloc<Vk.VkSemaphore>(waitSemaphores.Length);
            for (var i = 0; i < waitSemaphores.Length; i++)
                waitSemaphoresPtr[i] = waitSemaphores[i];
            submitInfo->pWaitSemaphores = waitSemaphoresPtr;
            var waitStages = scratch.Alloc<Vk.VkPipelineStageFlagBits>(waitSemaphores.Length);
            for (var i = 0; i < waitSemaphores.Length; i++)
                waitStages[i] = Vk.VkPipelineStageFlagBits.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
        ...
        var commandBuffersPtr = scratch.Alloc<Vk.VkCommandBuffer>(commandBuffers.Length);
        for (var i = 0; i < commandBuffers.Length; i++)
            commandBuffersPtr[i] = commandBuffers[i];
        submitInfo->pCommandBuffers = commandBuffersPtr;
        if (signalSemaphores.Length > 0)
        {
            submitInfo->signalSemaphoreCount = 1;
            var signalSemaphoresPtr = scratch.Alloc<Vk.VkSemaphore>();
            for (var i = 0; i < signalSemaphores.Length; i++)
                signalSemaphoresPtr[i] = signalSemaphores[i];
        ...
        var result = Vk.vkQueueSubmit(queue, 1, submitInfo, fence);
        if (result != Vk.VkResult.VK_SUCCESS)
            throw new Exception($"Failed to submit to queue: {result}");
    }

    public static void QueuePresent(Vk.VkQueue queue, Vk.VkSwapchainKHR swapchain, uint imageIndex,
        Vk.VkSemaphore[] waitSemaphores, MemUtils.Arena scratch)
    {
        var presentInfo = scratch.Alloc<Vk.VkPresentInfoKHR>();
        presentInfo->sType = Vk.VkStructureType.VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;
        presentInfo->pNext = null;
        if (waitSemaphores.Length > 0)
        {
            presentInfo->waitSemaphoreCount = 1;
            var waitSemaphoresPtr = scratch.Alloc<Vk.VkSemaphore>();
            for (var i = 0; i < waitSemaphores.Length; i++)
                waitSemaphoresPtr[i] = waitSemaphores[i];
        ...
        var swapchainsPtr = scratch.Alloc<Vk.VkSwapchainKHR>();
        swapchainsPtr[0] = swapchain;
        presentInfo->pSwapchains = swapchainsPtr;
        var imageIndicesPtr = scratch.Alloc<uint>();
        imageIndicesPtr[0] = imageIndex;
        presentInfo->pImageIndices = imageIndicesPtr;
        presentInfo->pResults = null;
        var result = Vk.vkQueuePresentKHR(queue, presentInfo);
        if (result != Vk.VkResult.VK_SUCCESS && result != Vk.VkResult.VK_SUBOPTIMAL_KHR)
    ...
    public struct ClearValue
    {
        ...
        public Vk.VkClearValue asVkClearValue()
        {
            var clearValue = new Vk.VkClearValue
            {
                color = new Vk.VkClearColorValue()
            };
